...
| 6 | Certificate distribution | Resolved | ESXi hosts: automated by `esxi_prepare` role. VCF components: step-ca signs CSRs via Phase 5 VCF Platform Services CA certificate replacement workflow. Keycloak: step-ca cert issued by `docker_services` role |
...
| R-004 | Two VCF domains (mgmt + wld) — follows "VCF Fleet in a Single Site with Minimal Footprint" blueprint + "Workload Domain Model: Simple" | NET-03, NET-04, ESX-02, VCF-01, VCF-02, VCF-03, VCF-04 | Phase 4 (§6) + Phase 6 (§7) | SDDC Manager shows both domains Active |
...
| R-012 | Shared-services VKS cluster for platform infrastructure | VKS-09, VKS-14 | Phase 8 — shared-services cluster deployment | `kubectl get nodes` on vks-services-01 shows 6 Ready |
| R-013 | Container registry proxy cache | VKS-11 | Phase 8 — Harbor installation | `curl https://harbor.lab.dreamfold.dev/api/v2.0/health` returns healthy |
| R-014 | L7 ingress routing for Kubernetes services | VKS-10 | Phase 8 — Contour installation | `kubectl get pods -n projectcontour` all Running |
...
| C-005 | Internet access for VKS/VCF sync | Deliver Guide §4.2 (gateway NAT/masquerade via Ansible gateway role) |
