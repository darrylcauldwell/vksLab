...
| Req. | Decision ID | Design Decision | Design Justification | Risk / Mitigation |
|------|-------------|-----------------|----------------------|-------------------|
| R-002 | SVC-07 | Keycloak as external OIDC identity provider, federated via VCF Identity Broker | Follows "Embedded VCF Identity Broker Model" + "Single VCF Instance SSO Model" — Identity Broker embedded in vCenter federates SSO across all VCF components; single sign-on reduces credential sprawl | Risk: Keycloak container outage blocks SSO login. Mitigation: Local administrator accounts remain functional as fallback; Keycloak container configured with restart policy |
...
| Design Decision | VCF 9 Design Library Model | Category |
|-----------------|---------------------------|----------|
| VCF-01: Domain separation | [VCF Fleet in a Single Site with Minimal Footprint](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/blueprints/vcf-fleet-basic-management-design.html) (management domain) + [Workload Domain Model: Simple](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/workload-domain-deployment-models/workload-domain-deployment-model.html) (consumption domain) | Blueprint |
| VCF-02: Single-node NSX Manager | [Simple NSX Manager Model](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/nsx-management-control-plane-design.html) — singleton appliance with vSphere HA | NSX management |
| VCF-03: VCF Operations | [Simple VCF Operations Model](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/vcf-operations-design/simple.html) | Operations |
| VCF-03: VCF Automation | [Simple VCF Automation Model](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/vcf-automation-deployment-models(1)/vcf-automation-simple-deployment-model.html) | Automation |
| ESX-03: vSAN ESA FTT=1 | [vSAN ESA Single-Rack HCI Model](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/storage-models.html) | Storage |
| Cluster topology | [Single-Rack Cluster Model](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/vsphere-cluster-design.html) — single cluster per domain | Cluster |
| NSX-01: Two-node Edge | [Host Fault Tolerant NSX Edge Model](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/design-library-nsx-edge-cluster/nsx-edge-cluster-models-for-single-availability-zone.html) — Active-Standby within single rack | NSX Edge |
...
| R-009 | VKS-15 | cert-manager ClusterIssuer integrated with step-ca via ACME | All Kubernetes ingress TLS certificates (Harbor, ArgoCD) are signed by the same lab CA as VCF component certificates (Phase 5); single trust chain from infrastructure through Kubernetes workloads | Risk: step-ca on the gateway is a single point of failure for certificate issuance. Mitigation: Acceptable for lab; step-ca auto-starts on gateway boot; cert-manager caches issued certs in Kubernetes secrets |
| R-015 | VKS-16 | Velero daily backup at 02:00, services namespace, 7-day retention with CSI snapshots | Daily captures PVC data (Harbor registry, ArgoCD state, MinIO); 7-day TTL balances recoverability against vSAN write amplification; scoped to services namespace as workload namespace contains transient test workloads | Risk: Daily backup may miss intra-day changes. Mitigation: Lab-grade RPO acceptable; manual `velero backup create` available for ad-hoc snapshots before risky changes |
| R-013 | VKS-17 | Harbor configured as GHCR proxy cache (`ghcr-proxy` project) | All lab microservices are published to GHCR; proxy cache avoids rate limits and reduces pull latency from the nested environment; Docker Hub images (nginx, minio/mc) are infrequent and do not need a proxy | Risk: GHCR outage blocks new image pulls. Mitigation: Cached images remain available; only new tags require upstream connectivity |
